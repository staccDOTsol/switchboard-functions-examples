...
const DEFAULT_COINTYPE: MoveStructTag = "0x1::aptos_coin::AptosCoin";
    ...
    this.intervalId = setInterval(async () => {
      try {
        ...
        if (events.length !== 0) {
          ...
          lastSequenceNumber = _.last(events).sequence_number;
        }
...
export class AggregatorAccount {
  constructor(
    ...
    readonly address: MaybeHexString,
    readonly switchboardAddress: MaybeHexString,
    readonly coinType: MoveStructTag = DEFAULT_COINTYPE
  ) {}
    ...
    return sendRawAptosTx(
      ...
      [
        new TxnBuilderTypes.TypeTagStruct(
          TxnBuilderTypes.StructTag.fromString(this.coinType)
        ),
      ],
      maxGasPrice
    );
  ...
  async openRound(
    ...
    return await sendAptosTx(
      ...
      [HexString.ensure(this.address).hex(), jitter],
      [this.coinType],
      maxGasPrice
    );
  }
  ...
  static async openRoundN(
    ...
    switchboardAddress: MaybeHexString,
    jitter: number = 1,
    coinType: MoveStructTag = DEFAULT_COINTYPE,
    maxGasPrice: number = 400
  ) {
    return await sendAptosTx(
      client,
      ...
      [aggregatorAddresses.map((addr) => HexString.ensure(addr).hex()), jitter],
      [coinType],
      maxGasPrice
    );
  }
...
export class EscrowManager {
  ...
  async loadData(): Promise<types.EscrowManager> {
    const data = (
      (await this.client.getAccountResource(
        this.address,
        `${this.switchboardAddress}::escrow::EscrowManager<${this.coinType}>`
      )) as any
    ).data;
...
export class OracleWallet {
  ...
  static async init(
    ...
    const oracleWallet = new OracleWallet(
      client,
      account.address(),
      switchboardAddress,
      params.coinType
