...
const solanaProgramDir = path.join(__dirname, "..");
...
async function main() {
  ...
  if (shouldBuild && isDev) {
    ...
    execSync(`anchor build`, { cwd: solanaProgramDir });
  }
  ...
  if (isDev) {
    spawn(
      ...
      { cwd: solanaProgramDir, stdio: "pipe" }
    );
...
async function runCommandAsync(command: string, options) {
