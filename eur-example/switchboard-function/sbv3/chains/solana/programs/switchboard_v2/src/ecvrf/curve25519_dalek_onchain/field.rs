...
pub use crate::ecvrf::curve25519_dalek_onchain::backend::serial::u64::field::*;
/// A `FieldElement` represents an element of the field
/// \\( \mathbb Z / (2\^{255} - 19)\\).
///
/// The `FieldElement` type is an alias for one of the platform-specific
/// implementations.
#[cfg(feature = "u64_backend")]
pub type FieldElement = crate::FieldElement51;
...
impl FieldElement {
    ...
    pub fn invert(&self) -> FieldElement {
        ...
        let (t3, t13, t15) = self.pow22501_p1(); // t19: 249..0 ; t3: 3,1,0
        sol_log_compute_units();
        let (t19, _) = Self::pow22501_p2(&t3, &t13, &t15);
    ...
    fn pow_p58_p1(&self) -> (FieldElement, FieldElement, FieldElement) {
        // msg!("pow_p58_p1");
    ...
    fn pow_p58_p2(
        ...
        //
        //                                 nonzero bits of exponent
        let (t19, _) = Self::pow22501_p2(t3, t13, t15); // 249..0
    ...
    fn pow22501_p1(&self) -> (FieldElement, FieldElement, FieldElement) {
        ...
        //
        let t0 = Box::new(self.square()); // 1         e_0 = 2^1
        let t1 = Box::new(t0.square().square()); // 3         e_1 = 2^3
        let t2 = Box::new(self * &*t1); // 3,0       e_2 = 2^3 + 2^0
        let t3 = Box::new(&*t0 * &*t2); // 3,1,0
        let t4 = Box::new(t3.square()); // 4,2,1
        let t5 = Box::new(&*t2 * &*t4); // 4,3,2,1,0
        let t6 = Box::new(t5.pow2k(5)); // 9,8,7,6,5
        let t7 = Box::new(&*t6 * &*t5); // 9,8,7,6,5,4,3,2,1,0
        let t8 = Box::new(t7.pow2k(10)); // 19..10
        let t9 = Box::new(&*t8 * &*t7); // 19..0
        let t10 = Box::new(t9.pow2k(20)); // 39..20
        let t11 = Box::new(&*t10 * &*t9); // 39..0
        let t12 = Box::new(t11.pow2k(10)); // 49..10
        let t13 = Box::new(&*t12 * &*t7); // 49..0
        let t14 = Box::new(t13.pow2k(50)); // 99..50
        let t15 = Box::new(&*t14 * &*t13); // 99..0
    ...
    fn pow22501_p2(
        ...
        // msg!("pow22501_p2");
        let t16 = Box::new(t15.pow2k(100)); // 199..100
        let t17 = Box::new(&*t16 * t15); // 199..0
        let t18 = Box::new(t17.pow2k(50)); // 249..50
        let t19 = Box::new(&*t18 * t13); // 249..0

        (*t19, *t3)
    }

    pub fn sqrt_ratio_i_p1(
        u: &FieldElement,
        v: &FieldElement,
        ...
        let v7 = &v3.square() * v;
        let l = &(u * &v7);
        let (t3, t13, t15) = l.pow_p58_p1();
        (t3, t13, t15)
    }

    pub fn sqrt_ratio_i_p2(
        u: &FieldElement,
        v: &FieldElement,
        t13: &FieldElement,
        t15: &FieldElement,
        ...
        let v3 = &v.square() * v;
        let v7 = &v3.square() * v;
        let l = &(u * &v7).pow_p58_p2(t13, t13, t15);
        Self::sqrip2i(u, v, &v3, &v7, l)
    }
    ...
    pub fn sqrip2i(
        ...
        let flipped_sign_sqrt_i = check.ct_eq(&(&(-u) * &constants::SQRT_M1));
    ...
    ///
    pub fn invsqrt_p1(&self) -> (FieldElement, FieldElement, FieldElement) {
        FieldElement::sqrt_ratio_i_p1(&FieldElement::one(), self)
    }
    pub fn invsqrt_p2(&self, y: &FieldElement, z: &FieldElement) -> (Choice, FieldElement) {
        // msg!("invsqrt_p2");
        FieldElement::sqrt_ratio_i_p2(&FieldElement::one(), self, y, z)
    }
}
//
// #[cfg(test)]
// mod test {
// use crate::ecvrf::curve25519_dalek_onchain::field::*;
// use subtle::ConditionallyNegatable;
//
