...
fn validate(ctx: &TaskRunnerContext, task: &MaxTask) -> Result<Vec<f64>, SbError> {
    let value = if let TaskOutput::Json(v) = &ctx.value {
        v
    } else {
        return Err(SbError::CustomMessage("max_task: Input value is not an array".to_string()));
    };
...
pub fn max_task(_ctx: &TaskRunnerContext, _task: &MaxTask) -> TaskResult<TaskOutput> {
