...
use crate::get_ixn_discriminator;
...
impl FunctionRunner {
    ...
    /// Generate instruction to upsert a feed owned by the given function
    /// Returns the feed pubkey and the upsert instrcution to be used in the
    /// function.
    pub fn upsert_feed(&self, name: &[u8; 32], value: Decimal) -> (Pubkey, Instruction) {
        #[derive(Clone, AnchorSerialize, AnchorDeserialize)]
        struct AggregatorFunctionUpsertParams {
            pub name: [u8; 32],
            pub value: BorshDecimal,
        }
        let (feed_key, _) = Pubkey::find_program_address(
            &[
                b"AggregatorAccountData",
                &self.function.to_bytes(),
                name,
            ],
            &SWITCHBOARD_PROGRAM_ID,
        );
        let mut data = get_ixn_discriminator("aggregator_function_upsert").to_vec();
        data.extend(
                AggregatorFunctionUpsertParams {
                    name: name.clone(),
                    value: BorshDecimal {
    ...
    pub async fn get_function_result(
        ...
        let blockhash = self.client.get_latest_blockhash_with_commitment(CommitmentConfig::confirmed())
            .map_err(|_| SbError::Message("Failed to get blockhash"))?.0;
    ...
    pub async fn emit(&self, ixs: Vec<Instruction>, commitment: Option<CommitmentConfig>) -> Result<(), SbError> {
        self.get_function_result(ixs, 0, commitment)
            .await
    ...
    pub async fn emit_error(&self, error_code: u8, commitment: Option<CommitmentConfig>) -> Result<(), SbError> {
        self.get_function_result(vec![], error_code, commitment)
            .await
            .unwrap()
            .emit();
        Ok(())
