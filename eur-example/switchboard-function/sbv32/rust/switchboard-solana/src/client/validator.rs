...
impl FunctionResultValidator {
    ...
    pub async fn load_from_cluster(
        ...
        let client =
            AnchorClient::new_with_options(cluster, payer.clone(), CommitmentConfig::processed());
    ...
    pub fn is_production(&self) -> bool {
        matches!(
            ...
            FunctionResultValidatorSigner::Production(_)
        )
    }
    ...
    pub fn is_simulation(&self) -> bool {
        matches!(
            &self.verifier_enclave_signer,
            FunctionResultValidatorSigner::Simulation(_)
        )
    }
    ...
    pub async fn process(&self, function_result: &FunctionResult) -> Result<Signature, SbError> {
        let (signature, _error_code) = match self.validate(function_result).await {
            ...
            Err(err) => {
                let function_pubkey =
                    Pubkey::try_from_slice(function_result.fn_key().unwrap_or_default().as_slice())
                        .unwrap_or_default();
                ...
                let error_code = match err {
                    ...
                    SbError::FunctionResultNonRetryableError(e) => {
                        println!(
                            "[QVN]({}) Failed with non-retryable error.\n{:?}",
                            function_pubkey, e
                        );
                        None
                    }
                    _ => {
                        println!(
                            "[QVN]({}) No error handler found for error {:?}",
                            function_pubkey, err
                        );
                ...
                if let Some(error_code) = error_code {
                    let mut tx = self
                        .produce_failover_tx(function_result, Some(error_code))
                        .await
                        .unwrap();
    ...
    pub async fn validate(&self, function_result: &FunctionResult) -> Result<Transaction, SbError> {
        ...
        let solana_function_result =
            if let Ok(switchboard_common::ChainResultInfo::Solana(chain_result_info)) =
        ...
        let function_pubkey =
            Pubkey::try_from_slice(function_result.fn_key().unwrap().as_slice()).unwrap();
        ...
        let (tx, request_type, untrusted_verify_idx) =
            self.build_and_verify_txn(&solana_function_result).await?;
        ...
        let untrusted_params =
            FunctionResultValidator::get_params(&request_type, verify_param_bytes.clone())?;
        ...
        if report_keyhash != Sha256::digest(function_enclave_signer.to_bytes()).as_slice() {
            ...
            return Err(SbError::FunctionResultFailoverError(
                200,
        ...
        if trusted_ix.data != untrusted_verify_ix.data {
            ...
            return Err(SbError::FunctionResultFailoverError(
                200,
                Arc::new(SbError::FunctionResultIxError(
                    "IllegalVerifyInstructionData",
                )),
            ));
        }
        ...
        let trusted_accounts: Vec<Pubkey> = trusted_ix.accounts.iter().map(|x| x.pubkey).collect();
        ...
        if trusted_accounts.len() != untrusted_accounts.len() {
            ...
            println!(
                "[QVN] {}: RIGHT: {:#?}",
                function_pubkey, untrusted_accounts
            );
            return Err(SbError::FunctionResultFailoverError(
                200,
        ...
        for (i, trusted_account) in trusted_accounts.iter().enumerate() {
            ...
            if untrusted_account != trusted_account
            ...
            .await
        {
            Ok(resp) => {
                ...
                if resp.value.err.is_some() {
                    println!("[QVN] function {} SimulationErrors: {:?}", function_pubkey, resp.value.err.unwrap());
                    println!("[QVN] function {} SimulationLogs: {:?}", function_pubkey, resp.value.logs.unwrap_or_default());
    ...
    async fn get_function_escrow_wallet(&self, function_pubkey: Pubkey) -> Result<Pubkey, SbError> {
        if let Some(timeout) = self.cache.timeout {
            ...
            self.cache
                .function_escrow_wallet
                .remove_if(&function_pubkey, |_k, entry| {
                    unix_timestamp() - entry.timestamp > timeout
                });
        }

        if let Some(function_escrow_cache_entry) =
            self.cache.function_escrow_wallet.get(&function_pubkey)
        {
            return Ok(function_escrow_cache_entry.pubkey);
        }

        let function_data = FunctionAccountData::fetch_async(&self.rpc, function_pubkey)
            .await
            .unwrap();

        self.cache.function_escrow_wallet.insert(
            function_pubkey,
            CacheEntry {
    ...
    async fn get_routine_escrow_wallet(&self, routine_pubkey: Pubkey) -> Result<Pubkey, SbError> {
        if let Some(timeout) = self.cache.timeout {
            let timeout: i64 = timeout.try_into().unwrap_or_default();
            self.cache
                .routine_escrow_wallet
                .remove_if(&routine_pubkey, |_k, entry| {
                    unix_timestamp() - entry.timestamp > timeout
                });
        }

        if let Some(routine_escrow_cache_entry) =
            self.cache.routine_escrow_wallet.get(&routine_pubkey)
        {
            return Ok(routine_escrow_cache_entry.pubkey);
        }

        let routine_data = FunctionRoutineAccountData::fetch_async(&self.rpc, routine_pubkey)
            .await
            .unwrap();

        self.cache.routine_escrow_wallet.insert(
            routine_pubkey,
            CacheEntry {
        ...
        let ix = FunctionVerify::build_ix(
            &(FunctionVerifyAccounts {
                ...
                function_enclave_signer: enclave_signer
                    .unwrap_or(verifier_accounts.verifier_enclave_signer),
                function_escrow,
    ...
    async fn build_request_verify_ix(
        ...
        let function_escrow = self.get_function_escrow_wallet(function).await?;
        let function_escrow_token_wallet =
            find_associated_token_address(&function_escrow, &NativeMint::ID);
    ...
    async fn produce_failover_tx(
        ...
        let verify_ixn: Instruction = match solana_function_result {
            ...
            SolanaFunctionResult::V1(v) => match v.request_type {
                SolanaFunctionRequestType::Routine(routine_pubkey_bytes) => {
                    let routine_pubkey = Pubkey::try_from_slice(&routine_pubkey_bytes[..]).unwrap();
                ...
                SolanaFunctionRequestType::Request(request_pubkey_bytes) => {
                    let request_pubkey = Pubkey::try_from_slice(&request_pubkey_bytes[..]).unwrap();
    ...
    async fn build_and_verify_txn(
        ...
        let tx: Transaction = bincode::deserialize(&solana_function_result.serialized_tx())
            .map_err(|_| {
                SbError::FunctionResultFailoverError(
                    200,
                    Arc::new(SbError::FunctionResultError(
        ...
        let untrusted_verify_pid_idx = tx
            .message
            .account_keys
            .iter()
            .position(|&x| x == SWITCHBOARD_ATTESTATION_PROGRAM_ID);
        if untrusted_verify_pid_idx.is_none()
        ...
        let request_type = match solana_function_result {
            ...
            SolanaFunctionResult::V0(_) => {
                (match ixn_discriminator {
                    FunctionVerify::DISCRIMINATOR => Ok(SolanaFunctionRequestType::Function(
                        tx.message.account_keys[untrusted_verify_ixn.accounts[0] as usize]
                            .to_bytes()
                            .to_vec(),
                    )),
                    FunctionRequestVerify::DISCRIMINATOR => Ok(SolanaFunctionRequestType::Request(
                        tx.message.account_keys[untrusted_verify_ixn.accounts[0] as usize]
                            .to_bytes()
                            .to_vec(),
                    )),
                    FunctionRoutineVerify::DISCRIMINATOR => Ok(SolanaFunctionRequestType::Routine(
                        tx.message.account_keys[untrusted_verify_ixn.accounts[0] as usize]
                            .to_bytes()
                            .to_vec(),
                    )),
                    _ => Err(SbError::FunctionResultFailoverError(
                        200,
        ...
        let compute_budget_pid = solana_sdk::compute_budget::ID;
        // @ottersec double check this is safe
        let feeds_pid = SWITCHBOARD_PROGRAM_ID;
        let functions_pid = SWITCHBOARD_ATTESTATION_PROGRAM_ID;
        ...
        if tx.message.instructions.len() > 1 {
            ...
            let payer_idx = tx
                ...
                .account_keys
                .iter()
                .position(|&x| x == self.payer.pubkey());
            let mut start_idx = 1;
            loop {
                if start_idx >= tx.message.instructions.len() {
                    break;
                }
                let pid = tx.message.account_keys[tx.message.instructions[start_idx].program_id_index as usize];
                if pid == compute_budget_pid || pid == feeds_pid || pid == functions_pid {
                    start_idx += 1;
                } else {
                    break;
                }
            }
            for ix in &tx.message.instructions[start_idx..] {
    ...
    fn get_params(
        ...
        match request_type {
            SolanaFunctionRequestType::Routine(_) => {
                let params =
                    FunctionRoutineVerifyParams::deserialize(&mut verify_param_bytes.as_slice())
                        .map_err(|_e| {
                            SbError::FunctionResultFailoverError(
                                200,
                                Arc::new(SbError::FunctionResultIxError("InvalidInstructionData")),
            ...
            SolanaFunctionRequestType::Request(_) => {
                let params =
                    FunctionRequestVerifyParams::deserialize(&mut verify_param_bytes.as_slice())
                        .map_err(|_e| {
                            SbError::FunctionResultFailoverError(
                                200,
                                Arc::new(SbError::FunctionResultIxError("InvalidInstructionData")),
            ...
            SolanaFunctionRequestType::Function(_) => {
                let params = FunctionVerifyParams::deserialize(&mut verify_param_bytes.as_slice())
                    .map_err(|_e| {
                        SbError::FunctionResultFailoverError(
                            200,
                            Arc::new(SbError::FunctionResultIxError("InvalidInstructionData")),
...
mod tests {
    ...
    fn build_quote_buffer(enclave_signer: &Keypair, mrenclave: [u8; 32]) -> Vec<u8> {
        ...
        // Set the report data
        quote_buffer[368..400]
            .copy_from_slice(Sha256::digest(enclave_signer.pubkey().to_bytes()).as_slice());
    ...
    async fn get_devnet_function_result_validator() -> &'static FunctionResultValidator {
        VALIDATOR.get_or_init(|| async {
            ...
            let keypair_path = home_dir.join(".config/solana/id.json"); // Build the path

            let payer = load_keypair_fs(keypair_path.to_str().unwrap()).unwrap();
    ...
    async fn setup_test_validator() -> &'static FunctionResultValidator {
        ...
        let payer_balance = validator
            .rpc
            .get_balance(&validator.payer.pubkey())
            .await
            .unwrap();
        ...
        if payer_balance == 0 {
            let sig = validator
                .rpc
                .request_airdrop(&validator.payer.pubkey(), 1_000_000_000)
                .await
                .unwrap();
    ...
    async fn test_function_validation() {
        ...
        match json_env_logger::try_init() {
            Ok(_) => {
        ...
        let sgx_quote_bytes =
            build_quote_buffer(&function_enclave_signer, DEFAULT_FUNCTION_MR_ENCLAVE);
        ...
        let function_result = FunctionResult::V1(FunctionResultV1 {
            ...
            signature: vec![],
            chain_result_info: ChainResultInfo::Solana(SolanaFunctionResult::V1(
    ...
    async fn test_request_validation() {
        ...
        let request_data = FunctionRequestAccountData::fetch_async(&validator.rpc, request_pubkey)
            .await
            .unwrap();
...
async fn print_function_verify_accounts(function_verify_ix: Instruction, rpc: Arc<RpcClient>) {
    let verify_ix_accounts: Vec<Pubkey> = function_verify_ix
        .accounts
        .clone()
        .iter()
        .map(|a| a.pubkey)
    ...
    println!(
        "#1 {:<24}: {:?}",
        "Function",
        verify_ix_accounts.get(0).unwrap()
    );
    println!(
        "#2 {:<24}: {:?}",
        "FunctionEnclaveSigner",
        verify_ix_accounts.get(1).unwrap()
    );
    println!(
        "#3 {:<24}: {:?}",
        "Verifier",
        verify_ix_accounts.get(2).unwrap()
    );
    println!(
        "#4 {:<24}: {:?}",
        "VerifierEnclaveSigner",
        verify_ix_accounts.get(3).unwrap()
    );
    println!(
        "#5 {:<24}: {:?}",
        "VerifierPermission",
        verify_ix_accounts.get(4).unwrap()
    );
    println!(
        "#6 {:<24}: {:?}",
        "EscrowWallet",
        verify_ix_accounts.get(5).unwrap()
    );
    println!(
        "#7 {:<24}: {:?}",
        "EscrowTokenWallet",
        verify_ix_accounts.get(6).unwrap()
    );
    println!(
        "#8 {:<24}: {:?}",
        "Receiver",
        verify_ix_accounts.get(7).unwrap()
    );
    println!(
        "#9 {:<24}: {:?}",
        "Attestation Queue",
        verify_ix_accounts.get(8).unwrap()
    );

    let account_infos = rpc
        .get_multiple_accounts(&verify_ix_accounts)
        .await
        .unwrap();
