...
// use crate::curve25519_dalek_onchain::backend::serial::scalar_mul::precomputed_straus::VartimePrecomputedStraus;

use crate::ecvrf::curve25519_dalek_onchain::edwards::EdwardsBasepointTable;
use crate::EdwardsPoint;
// use crate::ecvrf::curve25519_dalek_onchain::backend;
...
use super::traits::{MultiscalarMul, VartimeMultiscalarMul, VartimePrecomputedMultiscalarMul};
use crate::ecvrf::curve25519_dalek_onchain::traits::Identity;

use crate::EcvrfIntermediate;
#[cfg(all(
...
impl CompressedRistretto {
    ...
    pub fn decompress_fini(&self, I: &FieldElement) -> Option<RistrettoPoint> {
        let s = FieldElement::from_bytes(self.as_bytes());
        let s_bytes_check = s.to_bytes();
        let s_encoding_is_canonical = &s_bytes_check[..].ct_eq(self.as_bytes());
        let s_is_negative = s.is_negative();
    ...
    ///
    /// - `None` if `self` was not the canonical encoding of a point.
    pub fn decompress_m1(&self) -> (FieldElement, FieldElement) {
        // Step 1. Check s for validity:
        // 1.a) s must be 32 bytes (we get this from the type system)
        // 1.b) s < p
        // 1.c) s is nonnegative
        ...
        let tmp = &v * &u2_sqr;
        let (_x, y, z) = tmp.invsqrt_p1(); // 1/sqrt(v*u_2²)
        (y, z)
    }
    ...
    ///
    /// - `None` if `self` was not the canonical encoding of a point.
    pub fn decompress_m2(&self, y: &FieldElement, z: &FieldElement) -> Option<RistrettoPoint> {
        // Step 1. Check s for validity:
        // 1.a) s must be 32 bytes (we get this from the type system)
        // 1.b) s < p
        // 1.c) s is nonnegative
        ...
        let tmp = &v * &u2_sqr;
        let (ok, I) = tmp.invsqrt_p2(y, z); // 1/sqrt(v*u_2²)

        // msg!("decompress_m2_l2");
        let Dx = &I * &u2; // 1/sqrt(v)
        let Dy = &I * &(&Dx * &v); // 1/u2
...
unsafe impl Pod for RistrettoPoint {}
unsafe impl Zeroable for RistrettoPoint {}
...
impl RistrettoPoint {
    ...
    pub fn compress_p1(
        ...
        let u1: FieldElement = &(Z + &Y) * &(Z - &Y);
        let u2: FieldElement = &X * &Y;
        // Ignore return value since this is always square
        let invertee: FieldElement = &u1 * &u2.square();
        let (_x, y, z) = invertee.invsqrt_p1();
        (u1, u2, invertee, y, z)
    }
    ...
    pub fn compress_p2(
        ...
        let (_, invsqrt) = invertee.invsqrt_p2(y, z);
        let i1 = &invsqrt * u1;
        let i2 = &invsqrt * u2;
        let z_inv = &i1 * &(&i2 * T);
        let mut den_inv = i2;
    ...
    pub(crate) fn elligator_ristretto_flavor_p1(
        r_0: &FieldElement,
        ...
        let r = i * &r_0.square();
        let N_s: FieldElement = &(&r + &one) * one_minus_d_sq;
        let D: FieldElement = &(&c - &(d * &r)) * &(&r + d);

        // msg!("Q");
        // TODO: (mgild) storg N_s, D, t13, t15
        let (_t3, t13, t15) = FieldElement::sqrt_ratio_i_p1(&N_s, &D);
        (N_s, D, t13, t15)
    }

    pub(crate) fn elligator_ristretto_flavor_p2(
        r_0: &FieldElement,
        N_s: FieldElement,
        D: FieldElement,
        t13: FieldElement,
        ...
        let one = FieldElement::one();
        // let (Ns_D_is_sq, mut s) = FieldElement::sqrt_ratio_i_p2(&N_s, &D, &t13, &t15);
        let (Ns_D_is_sq, mut s) = FieldElement::sqrt_ratio_i_p2(&N_s, &D, &t13, &t15);
    ...
    pub fn random<R>(_rng: &mut R) -> Self {
        let output_bytes = [0u8; 64];
        // rng.fill_bytes(&mut uniform_bytes);

        let ei = RistrettoPoint::from_uniform_bytes_p1(&output_bytes[..32]);
        let R_1 = RistrettoPoint::from_uniform_bytes_p2(
            ei.r.into(),
            ei.N_s.into(),
            ei.D.into(),
            ei.t13.into(),
        ...
        let ei = RistrettoPoint::from_uniform_bytes_p1(&output_bytes[32..64]);
        let R_2 = RistrettoPoint::from_uniform_bytes_p2(
            ei.r.into(),
            ei.N_s.into(),
            ei.D.into(),
            ei.t13.into(),
    ...
    pub fn from_uniform_bytes_p1(bytes: &[u8]) -> EcvrfIntermediate {
        let mut r_bytes = [0u8; 32];
        r_bytes.copy_from_slice(&bytes[..32]);
        let r = FieldElement::from_bytes(&r_bytes);
        let (N_s, D, t13, t15) = RistrettoPoint::elligator_ristretto_flavor_p1(&r);
        EcvrfIntermediate {
            r: r.into(),
            N_s: N_s.into(),
            D: D.into(),
    ...
    pub fn from_uniform_bytes_p2(
        ...
        RistrettoPoint::elligator_ristretto_flavor_p2(&r, N_s, D, t13, t15)
    }
    // pub fn from_uniform_bytes_p4(bytes: &[u8; 64]) -> (FieldElement, FieldElement, FieldElement, FieldElement) {
    // let mut r_1_bytes = [0u8; 32];
    // r_1_bytes.copy_from_slice(&bytes[0..32]);
    // msg!("AAA");
    // let r_1 = FieldElement::from_bytes(&r_1_bytes);
    // msg!("AAA");
    // let (N_s, D, t13, t15) = RistrettoPoint::elligator_ristretto_flavor_p1(&r_1);
    // let R_1 = RistrettoPoint::elligator_ristretto_flavor_p2(&r_1, N_s, D, t13, t15);
    // msg!("AAA");
    //
    // let mut r_2_bytes = [0u8; 32];
    // r_2_bytes.copy_from_slice(&bytes[32..64]);
    // let r_2 = FieldElement::from_bytes(&r_2_bytes);
    // let (N_s, D, t13, t15) = RistrettoPoint::elligator_ristretto_flavor_p1(&r_2);
    // let R_2 = RistrettoPoint::elligator_ristretto_flavor_p2(&r_2, N_s, D, t13, t15);
    // msg!("AAA");
    //
    // // Applying Elligator twice and adding the results ensures a
    // // uniform distribution.
...
// outer type.
#[cfg(feature = "alloc")]
pub struct VartimeRistrettoPrecomputation(crate::curve25519_dalek_onchain::backend::serial::scalar_mul::precomputed_straus::VartimePrecomputedStraus);
...
impl VartimePrecomputedMultiscalarMul for VartimeRistrettoPrecomputation {
    ...
    {
        Self(VartimePrecomputedStraus::new(
