...
export const SWITCHBOARD_DEVNET_ADDRESS = `0xfd2e0f4383df3ec9106326dcd9a20510cdce72146754296deed15403fcd3df8b`;
export const SWITCHBOARD_TESTNET_ADDRESS = `0x4247e72df58552701456293027e75237fe85a214cd050b6e0358dc5047a3fb17`;
...
export class JobAccount {
  ...
  async loadJob(): Promise<OracleJob> {
    ...
    const job = await Promise.resolve()
      .then(() => OracleJob.decodeDelimited(Buffer.from(data.data, "base64")))
      .catch(() => {
        const buffer = Buffer.from(data.data).toString();
        return OracleJob.decodeDelimited(Buffer.from(buffer, "base64"));
      });
    return job;
  }
