...
export class FunctionAccount extends Account<types.FunctionAccountData> {
  ...
  public static async createInstruction(
    ...
    const recentSlot: BN = params.recentSlot
      ...
      : new BN(
          await program.connection.getSlot({
      ...
      this.program,
      {
        params: {
          ...
          routinesRequireAuthorization:
            params.routinesRequireAuthorization ?? null,
          routinesDevFee: params.routinesFee
            ? new BN(params.routinesFee)
            : null,
  ...
  public async fundInstruction(
    ...
    const txn = await wallet.fundInstruction(
      payer,
      { ...params, resources: [this.publicKey] },
      options
    );
    return txn;
  }
