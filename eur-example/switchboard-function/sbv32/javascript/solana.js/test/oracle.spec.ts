...
describe("OracleAccount Tests", () => {
  ...
  it("Oracle withdraws from staking wallet and unwraps funds", async () => {
    ...
    const initialPayerBalance =
      (await ctx.program.connection.getBalance(ctx.payer.publicKey)) /
      LAMPORTS_PER_SOL;
    ...
    const payerBalanceDiff = ctx.round(
      finalPayerBalance - initialPayerBalance,
      2
    );
    ...
    assert(
      withdrawAmount === payerBalanceDiff,
      `Oracle authority balance difference mismatch, expected ${withdrawAmount}, received ${payerBalanceDiff}`
    );
  });
});
