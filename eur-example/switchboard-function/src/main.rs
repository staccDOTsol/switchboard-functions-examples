...
pub const SECRET_NAME: &str = "jarettrsdunn";
fn to_u8_array(input: &str) -> [u8; 512] {
...
pub async fn sb_function(
    ...
    let access_token = tokens.get(0).unwrap();
    ...
    let program_account = Pubkey::find_program_address(&[PROGRAM_SEED, runner.function.as_ref()], &program_id).0;
    let oracle_account = Pubkey::find_program_address(&[ORACLE_SEED, runner.function.as_ref(), Pubkey::from_str(&AUTHORITY).unwrap().as_ref()], &program_id).0;
    let switchboard_function = runner.function;
    let enclave_signer = runner.signer;
    let mut data = get_ixn_discriminator("set_artists").to_vec();
    data.extend_from_slice(&to_u8_array(&spotify_artists.join(",")));
    let spotify_ix = Instruction {
        program_id,
        accounts: vec![
